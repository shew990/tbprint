...
{
    public partial class T_PickCar_DB : BILBasic.Basing.Base_DB<T_PickCarInfo>
    {

        /// <summary>
        /// 添加t_pickcar
        /// </summary>
        protected override OracleParameter[] GetSaveModelOracleParameter(T_PickCarInfo t_pickcar)
        {
        ...
        /// 将获取的单条数据转封装成对象返回
        /// </summary>
        protected override T_PickCarInfo ToModel(OracleDataReader reader)
        {
            T_PickCarInfo t_pickcar = new T_PickCarInfo();

            t_pickcar.ID = OracleDBHelper.ToModelValue(reader, "ID").ToInt32();
            t_pickcar.CarNo = (string)OracleDBHelper.ToModelValue(reader, "CARNO");
            t_pickcar.TaskNo = (string)OracleDBHelper.ToModelValue(reader, "TASKNO");
            t_pickcar.Creater = (string)OracleDBHelper.ToModelValue(reader, "CREATER");
            t_pickcar.CreateTime = (DateTime?)OracleDBHelper.ToModelValue(reader, "CREATETIME");
            t_pickcar.Modifyer = (string)OracleDBHelper.ToModelValue(reader, "MODIFYER");
            t_pickcar.ModifyTime = (DateTime?)OracleDBHelper.ToModelValue(reader, "MODIFYTIME");
            t_pickcar.IsDel = OracleDBHelper.ToModelValue(reader, "IsDel").ToInt32();
            return t_pickcar;
        }
