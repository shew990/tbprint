...
{
    public partial class T_MouldType_DB : BILBasic.Basing.Base_DB<T_MouldType>
    {
        ...
        {
            object id = OracleDBHelper.ExecuteScalar(System.Data.CommandType.Text, "SELECT COUNT(*) FROM Mes_ModelType WHERE ID='" + model.MouldTypeCode + "'");
        ...
        /// 将获取的单条数据转封装成对象返回
        /// </summary>
        protected override T_MouldType ToModel(OracleDataReader reader)
        {
            T_MouldType t_customer = new T_MouldType();

            t_customer.MouldTypeCode = OracleDBHelper.ToModelValue(reader, "ID").ToDBString();
            t_customer.MouldTypeName = OracleDBHelper.ToModelValue(reader, "NAME").ToDBString();
            t_customer.MachineType = OracleDBHelper.ToModelValue(reader, "machineType").ToDBString();
            t_customer.Qty = OracleDBHelper.ToModelValue(reader, "qty").ToInt32();
        ...
        protected override OracleParameter[] GetSaveModelOracleParameter(T_MouldType model)
        {
