...
{
    public partial class T_Modl_DB : BILBasic.Basing.Base_DB<T_Modl>
    {

        /// <summary>
        /// 添加t_customer
        /// </summary>
        protected override OracleParameter[] GetSaveModelOracleParameter(T_Modl t_customer)
        {
            ...
            OracleParameter[] param = new OracleParameter[]{
              new OracleParameter("@bResult",OracleDbType.Int32),
               ...
               new OracleParameter("@v_MouldCode", OracleDBHelper.ToDBValue(t_customer.MouldCode).ToOracleValue()),
        ...
        /// 将获取的单条数据转封装成对象返回
        /// </summary>
        protected override T_Modl ToModel(OracleDataReader reader)
        {
            T_Modl t_customer = new T_Modl();

            t_customer.ID = OracleDBHelper.ToModelValue(reader, "ID").ToInt32();
            t_customer.Sn = (string)OracleDBHelper.ToModelValue(reader, "SN");
            t_customer.MouldCode = (string)OracleDBHelper.ToModelValue(reader, "MOULDCODE");
            t_customer.MouldName = (string)OracleDBHelper.ToModelValue(reader, "MOULDNAME");
            t_customer.MouldType = OracleDBHelper.ToModelValue(reader, "MOULDTYPE").ToDBString();
            t_customer.Category = OracleDBHelper.ToModelValue(reader, "Category").ToDBString();
            t_customer.MachineType = OracleDBHelper.ToModelValue(reader, "MachineType").ToDBString();
            t_customer.MouldStatus = OracleDBHelper.ToModelValue(reader, "MOULDTYPE").ToDBString();
            t_customer.ClearStatus = OracleDBHelper.ToModelValue(reader, "ClearStatus").ToDBString();
            t_customer.Disinfection = OracleDBHelper.ToModelValue(reader, "Disinfection").ToDBString();
            t_customer.ULQty = OracleDBHelper.ToModelValue(reader, "ULQty").ToDBString();
            t_customer.Acupoints = OracleDBHelper.ToModelValue(reader, "Acupoints").ToDBString();
            t_customer.ProductType = OracleDBHelper.ToModelValue(reader, "ProductType").ToDBString();
            t_customer.Texture = OracleDBHelper.ToModelValue(reader, "Texture").ToDBString();
            t_customer.OutSize = OracleDBHelper.ToModelValue(reader, "OutSize").ToDBString();
            t_customer.Unit = OracleDBHelper.ToModelValue(reader, "Unit").ToDBString();
            t_customer.OpenSize = OracleDBHelper.ToModelValue(reader, "OpenSize").ToDBString();
            t_customer.Remark = (string)OracleDBHelper.ToModelValue(reader, "REMARK");
            t_customer.ERPNote = (string)OracleDBHelper.ToModelValue(reader, "SumQty");//模具总数量
            t_customer.TaskNo = (string)OracleDBHelper.ToModelValue(reader, "DelQty");//模具报废数量
        ...
        private int GetID()
        {
            object id = OracleDBHelper.ExecuteScalar(System.Data.CommandType.Text, "SELECT MAX(ID) FROM Mes_Mould");
        ...
        {
            object id = OracleDBHelper.ExecuteScalar(System.Data.CommandType.Text, "SELECT COUNT(*) FROM Mes_Mould WHERE MouldCode='" + model.MouldCode + "'");
