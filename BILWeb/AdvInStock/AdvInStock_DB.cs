...
{
    public partial class T_AdvInStock_DB : BILBasic.Basing.Base_DB<T_AdvInStockInfo>
    {

        /// <summary>
        /// 添加t_advinstock
        /// </summary>
        protected override OracleParameter[] GetSaveModelOracleParameter(T_AdvInStockInfo t_advinstock)
        {
    ...
    "values (" + t_advinstock.ID + ", '" + t_advinstock.VoucherNo + "', '" + t_advinstock.ErpVoucherNo + "', '" + t_advinstock.SupplierNo + "','" + t_advinstock.SupplierName + "'," + t_advinstock.VoucherType + ", sysdate, '" + t_advinstock.Creater + "', '" + t_advinstock.Modifyer + "',sysdate, " + t_advinstock.Status + ", 1, '" + t_advinstock.StrongHoldCode + "', '" + t_advinstock.StrongHoldName + "', '" + t_advinstock.CompanyCode + "', '" + t_advinstock.WarehouseID + "')";
            ...
            {
                lstSql.Add("update t_instockdetail set ADVRECEIVEQTY=nvl(ADVRECEIVEQTY,0)+" + receiveQty[item] + " where id=" + item);
            }
            lstSql.Add("update t_Instock a set advinstatus =3 where (select count(*) from t_Instock a left join t_instockdetail d on a.id =d.headerid where d.instockqty >nvl(d.advreceiveqty,0) and a.erpvoucherno ='" + t_advinstock.ErpVoucherNo + "')=0 and a.erpvoucherno ='" + t_advinstock.ErpVoucherNo + "' ");
            return lstSql;
        }
        /// <summary>
        /// 获取质检类型
        ...
        public List<Parameter_Model> GetAdvInParameter(string groupname)
        {
            ...
            using (OracleDataReader dr = OracleDBHelper.ExecuteReader(CommandType.Text, "select parameterid,parametername from t_parameter where groupname ='" + groupname + "'"))
            {
                while (dr.Read())
                {
        ...
        /// 将获取的单条数据转封装成对象返回
        /// </summary>
        protected override T_AdvInStockInfo ToModel(OracleDataReader reader)
        {
            T_AdvInStockInfo t_advinstock = new T_AdvInStockInfo();

            t_advinstock.ID = (int)OracleDBHelper.ToModelValue(reader, "ID");
            t_advinstock.VoucherNo = (string)OracleDBHelper.ToModelValue(reader, "VOUCHERNO");
            t_advinstock.ErpVoucherNo = (string)OracleDBHelper.ToModelValue(reader, "ERPVOUCHERNO");
            t_advinstock.SupplierNo = (string)OracleDBHelper.ToModelValue(reader, "SUPPLIERNO");
            t_advinstock.SupplierName = (string)OracleDBHelper.ToModelValue(reader, "SUPPLIERNAME");
            t_advinstock.VoucherType = (int)OracleDBHelper.ToModelValue(reader, "VOUCHERTYPE");
            t_advinstock.CreateTime = (DateTime?)OracleDBHelper.ToModelValue(reader, "CREATETIME");
            t_advinstock.Creater = (string)OracleDBHelper.ToModelValue(reader, "CREATER");
            t_advinstock.Modifyer = (string)OracleDBHelper.ToModelValue(reader, "MODIFYER");
            t_advinstock.ModifyTime = (DateTime?)OracleDBHelper.ToModelValue(reader, "MODIFYTIME");
            t_advinstock.Status = (int)OracleDBHelper.ToModelValue(reader, "STATUS");
            t_advinstock.IsDel = (decimal?)OracleDBHelper.ToModelValue(reader, "ISDEL");
            t_advinstock.StrongHoldCode = (string)OracleDBHelper.ToModelValue(reader, "STRONGHOLDCODE");
            t_advinstock.StrongHoldName = (string)OracleDBHelper.ToModelValue(reader, "STRONGHOLDNAME");
            t_advinstock.CompanyCode = (string)OracleDBHelper.ToModelValue(reader, "COMPANYCODE");
            t_advinstock.WarehouseID = (int)OracleDBHelper.ToModelValue(reader, "WAREHOUSEID");
            return t_advinstock;
        }
