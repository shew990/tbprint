    ...
    public partial class T_OutStockDetail_Func : TBase_Func<T_OutStockDetail_DB, T_OutStockDetailInfo>
     {
        ...
        public string GetReviewStockModelADF(string ModelStockJson)
        {
            ...
            try
            {
                ...
                int iResult = _db.CheckBarCodeIsReview(strSerialNo);
                if (iResult > 0) 
                {
                    messageModel.HeaderStatus = "E";
                    messageModel.Message = "批次标签已经复核，不能重复扫描复核！";
                    return JsonConvert.SerializeObject(messageModel);
                }
        ...
        /// <param name="strError"></param>
        /// <returns></returns>
        public bool GetReviewStockModel(UserModel user,T_StockInfo model,ref List<T_StockInfo> modelList,ref List<T_OutStockDetailInfo> outStockDetailList, ref string strError)
        {
            bool bCheck = false;
        ...
        /// <param name="strError"></param>
        /// <returns></returns>
        public bool SaveT_OutStockReviewDetailENA(UserModel user, T_StockInfo model, ref List<T_OutStockDetailInfo> outStockDetailList, ref string strError)
        {
            try
            {
                List<T_OutStockTaskDetailsInfo> modelListTaskDetail = new List<T_OutStockTaskDetailsInfo>();
        ...
        private bool CheckReviewOutStockStatus(ref string strError, T_OutStockInfo outStockModel)
        {
            ...
            {
                strError = "该订单已经过账，不能再次复核";
                return false;
            }

            if (outStockModel.Status == 4)
            {
                strError = "该订单已经复核完成，不能再次复核";
                return false;
            }
            return true;
        }
        ...
        public List<T_StockInfo> GroupByStockQtyByEan(List<T_StockInfo> modelList) 
        {
            var groupList = from t in modelList
                            group t by new { t1 = t.MaterialNoID,t2 = t.TaskDetailesID } into m
