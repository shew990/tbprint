...
{
    public partial class T_SerialNo_DB : BILBasic.Basing.Base_DB<T_SerialNoInfo>
    {

        /// <summary>
        /// 添加t_serialno
        /// </summary>
        protected override OracleParameter[] GetSaveModelOracleParameter(T_SerialNoInfo t_serialno)
        {
        ...
        /// 将获取的单条数据转封装成对象返回
        /// </summary>
        protected override T_SerialNoInfo ToModel(OracleDataReader reader)
        {
            T_SerialNoInfo t_serialno = new T_SerialNoInfo();

            t_serialno.ID = OracleDBHelper.ToModelValue(reader, "ID").ToInt32();
            t_serialno.FacMaterialNo = (string)OracleDBHelper.ToModelValue(reader, "FACMATERIALNO");
            t_serialno.SerialNo = (string)OracleDBHelper.ToModelValue(reader, "SERIALNO");
            t_serialno.ERPVoucherNo = (string)OracleDBHelper.ToModelValue(reader, "ERPVOUCHERNO");
            t_serialno.TaskNo = OracleDBHelper.ToModelValue(reader, "TaskNo").ToDBString();
            t_serialno.VoucherNo = OracleDBHelper.ToModelValue(reader, "VoucherNo").ToDBString();
