...
{
    public partial class t_RuleAll_DB : BILBasic.Basing.Base_DB<T_RuleAllInfo>
    {

        /// <summary>
        /// 添加t_ruleall
        /// </summary>
        protected override OracleParameter[] GetSaveModelOracleParameter(T_RuleAllInfo t_ruleall)
        {
        ...
        /// 将获取的单条数据转封装成对象返回
        /// </summary>
        protected override T_RuleAllInfo ToModel(OracleDataReader reader)
        {
            T_RuleAllInfo t_ruleall = new T_RuleAllInfo();

            t_ruleall.ID = OracleDBHelper.ToModelValue(reader, "ID").ToInt32();
            t_ruleall.IsDel = OracleDBHelper.ToModelValue(reader, "ISDEL").ToInt32();
            t_ruleall.Creater = (string)OracleDBHelper.ToModelValue(reader, "CREATER");
            t_ruleall.CreateTime = (DateTime?)OracleDBHelper.ToModelValue(reader, "CREATETIME");
            t_ruleall.Modifyer = (string)OracleDBHelper.ToModelValue(reader, "MODIFYER");
            t_ruleall.ModifyTime = (DateTime?)OracleDBHelper.ToModelValue(reader, "MODIFYTIME");
            t_ruleall.ConItemID = OracleDBHelper.ToModelValue(reader, "CONITEMID").ToInt32();
            t_ruleall.IsEnable = OracleDBHelper.ToModelValue(reader, "ISENABLE").ToInt32();
            t_ruleall.strConItemID = (string)OracleDBHelper.ToModelValue(reader, "strConItemID");
            t_ruleall.strIsEnable = (string)OracleDBHelper.ToModelValue(reader, "strIsEnable");
        ...
        /// <returns></returns>
        public override List<T_RuleAllInfo> GetModelListByPage(UserModel user, T_RuleAllInfo model, ref DividPage page)
        {
            //object obj = ExampleCache.GetExampleCache().Get("ruleall");
            ////设置缓存
            //if (obj != null)
            ...
            List<T_RuleAllInfo> modelList = base.GetModelListByPage(user, model, ref page);
