...
{
    public class ParamaterFiled_DB : BILBasic.Basing.Base_DB<ParamaterField_Model>
    {
        protected override OracleParameter[] GetSaveModelOracleParameter(ParamaterField_Model model)
        {
        ...
        protected override ParamaterField_Model ToModel(OracleDataReader reader)
        {
            ParamaterField_Model pmodel = new ParamaterField_Model();
            pmodel.ID = OracleDBHelper.ToModelValue(reader, "ID").ToInt32();
            pmodel.InStockType = OracleDBHelper.ToModelValue(reader, "InStockType").ToInt32();
            pmodel.VoucherType = OracleDBHelper.ToModelValue(reader, "VoucherType").ToInt32();
            pmodel.ErpVouType = OracleDBHelper.ToModelValue(reader, "ErpVouType").ToInt32();
            pmodel.WMSTableNameH = OracleDBHelper.ToModelValue(reader, "WMSTableNameH").ToDBString();
            pmodel.WMSTableNameD = OracleDBHelper.ToModelValue(reader, "WMSTableNameD").ToDBString();
            pmodel.HEADID = OracleDBHelper.ToModelValue(reader, "HEADID").ToDBString();
            pmodel.DETAILID = OracleDBHelper.ToModelValue(reader, "DETAILID").ToDBString();
            pmodel.VOUCHERNO = OracleDBHelper.ToModelValue(reader, "VOUCHERNO").ToDBString();
            pmodel.ExcelTitleLangrage = OracleDBHelper.ToModelValue(reader, "ExcelTitleLangrage").ToDBString();
            pmodel.FieldHD = OracleDBHelper.ToModelValue(reader, "FieldHD").ToString();
            pmodel.XlsNameEN = OracleDBHelper.ToModelValue(reader, "XlsNameEN").ToDBString();
            pmodel.XlsNameCN = OracleDBHelper.ToModelValue(reader, "XlsNameCN").ToDBString();
            pmodel.WMSField = OracleDBHelper.ToModelValue(reader, "WMSField").ToDBString();
            pmodel.ERPField = OracleDBHelper.ToModelValue(reader, "ERPField").ToDBString();
            pmodel.DefaultValue = OracleDBHelper.ToModelValue(reader, "DefaultValue").ToDBString();
            pmodel.HEADKEYS = OracleDBHelper.ToModelValue(reader, "HEADKEYS").ToDBString();
            pmodel.DETAILKEYS = OracleDBHelper.ToModelValue(reader, "DETAILKEYS").ToDBString();
            pmodel.DefaultType = OracleDBHelper.ToModelValue(reader, "DefaultType").ToInt32();
            pmodel.ErpVourcherType = OracleDBHelper.ToModelValue(reader, "ErpVourcherType").ToDBString();
            pmodel.CompanyNo = OracleDBHelper.ToModelValue(reader, "CompanyNo").ToDBString();
            pmodel.MATERIALNOKEYS = OracleDBHelper.ToModelValue(reader, "MATERIALNOKEYS").ToDBString();
            return pmodel;
        }
        ...
        private void CreateSqlByJson(List<ParamaterField_Model> pmList, string syncType, ref string sql, List<string> sqlList, ref List<deleteModel> deleteModels, JObject jObject, int VoucherType, List<ParamaterField_Model> pmListbyType)
        {
            ...
            int HeadtaskID = base.GetTableID(pmListbyType[0].HEADID); //表头ID序号
           ...
           ref List<deleteModel> deleteModels)
        {
            ...
            string sqlInsertTitle = "insert when (not exists (@)) then into {0}(ID,VOUCHERNO,VOUCHERTYPE,{4}) values({1},{2},{3},{5}) select * from  dual";
            string sqlInsertDetail = "insert when (not exists (@)) then into {0}(ID,HEADERID,VOUCHERNO,{4}) values({1},{2},{3},{5}) select * from  dual";
           ...
           var defaultValueList =pmList.FindAll(
            ...
            sql =                       
                ...
                valueList.Remove(valueList.Length - 1, 1)) :
                 String.Format(sql, tableName, base.GetTableID(pmList[0].DETAILID), HeadtaskID, (WmsVourcherNo.Equals("") ? HeadtaskID + "" : WmsVourcherNo), contextList.Remove(contextList.Length - 1, 1),
                valueList.Remove(valueList.Length - 1, 1));
            return sql;
        }
        ...
        internal void insertStock(string dataJson, ref string errMsg)
        {
            ...
            for (int i = 0; i < jarray.Count; i++)
            {
                ...
                DataTable dr= OracleDBHelper.ExecuteDataSet(CommandType.Text,sqlselet).Tables[0];
        ...
        //        OracleDataReader reader = base.GetRowBySql(getInsertInfo);
        //        if (reader.Read())
        //        {
