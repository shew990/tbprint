    ...
    public class Check_DB
    {
        ...
        public bool GetCheckAnalyze(CheckAnalyze taskmo, ref DividPage dividpage, ref List<CheckAnalyze> lsttask, ref string strErrMsg)
        {
            ...
            try
            {
                ...
                string sql = "select ROW_NUMBER() OVER(Order by INSERTTIME) AS PageRowNumber ,a.creater,a.status,a.materialno,a.materialdesc,a.strongholdcode,a.INSERTTIME,a.CHECKNO,a.AREANO,a.QTY,a.SERIALNO," +
                            ...
                            "case when nvl(a.QTY,0)<nvl(b.QTY,0) then '亏' when nvl(a.QTY,0)>nvl(b.QTY,0)  then '赢' else '平'" +
                            " end as remark,case when nvl(a.QTY,0)>nvl(b.QTY,0) then to_char(nvl(a.QTY,0)-nvl(b.QTY,0)) else '' end as YQTY,case when nvl(a.QTY,0)<nvl(b.QTY,0) then to_char(nvl(b.QTY,0)-nvl(a.QTY,0)) else '' end as KQTY "
                ...
                if (taskmo.remark != "全部")
                    sql += " and case when nvl(a.QTY,0)<nvl(b.QTY,0) then '亏' when nvl(a.QTY,0)>nvl(b.QTY,0)  then '赢' else '平' end ='" + taskmo.remark + "' order by a.materialno";
                using (OracleDataReader dr = Common_DB.QueryByDividPage2(ref dividpage, sql))
                {
                    while (dr.Read())
                    {
        ...
        public CheckAnalyze GetCheckAnalyze_GetModelFromDataReader(OracleDataReader dr)
        {
        ...
        public bool GetCheckAnalyze2(CheckAnalyze taskmo, ref DividPage dividpage, ref List<CheckAnalyze> lsttask, ref string strErrMsg)
        {
            ...
            try
            {
                ...
                string sql = "select ROW_NUMBER() OVER(Order by INSERTTIME) AS PageRowNumber,a.materialno,a.materialdesc,a.strongholdcode,a.AREANO,a.QTY," +
                            ...
                            "(case when nvl(a.QTY,0)<nvl(b.QTY,0) then '亏' when  nvl(a.QTY,0)>nvl(b.QTY,0)then '赢' else '平' end)" +
                            " as remark,case when (case when nvl(a.QTY,0)<nvl(b.QTY,0) then '亏' when  nvl(a.QTY,0)>nvl(b.QTY,0)then '赢' else '平' end) ='赢' then  to_char((nvl(a.QTY,0)-nvl(b.QTY,0))) else '' end as YQTY," +
                            "case when (case when nvl(a.QTY,0)<nvl(b.QTY,0) then '亏' when  nvl(a.QTY,0)>nvl(b.QTY,0)then '赢' else '平' end) ='亏' then to_char((nvl(b.QTY,0)-nvl(a.QTY,0))) else '' end as KQTY from " +
                ...
                if (taskmo.remark != "全部")
                    sql += " and case when nvl(a.QTY,0)<nvl(b.QTY,0) then '亏' when  nvl(a.QTY,0)>nvl(b.QTY,0)then '赢' else '平' end ='" + taskmo.remark + "'";
                using (OracleDataReader dr = Common_DB.QueryByDividPage2(ref dividpage, sql))
                {
                    while (dr.Read())
                    {
        ...
        public CheckAnalyze GetCheckAnalyze_GetModelFromDataReader2(OracleDataReader dr)
        {
            CheckAnalyze TMM = new CheckAnalyze();
        ...
        public bool GetCheckInfo(Check_Model taskmo, ref DividPage dividpage, ref List<Check_Model> lsttask, ref string strErrMsg)
        {
            ...
            try
            {
                using (OracleDataReader dr = Common_DB.QueryByDividPage(ref dividpage, "T_CHECK", CHECK_GetFilterSql(taskmo), " * ", "Order by ID Desc"))
                {
                    while (dr.Read())
                    {
        ...
        private string CHECK_GetFilterSql(Check_Model mo)
        {
            ...
            {
                sql += "and CREATETIME>=to_date('" + mo.begintime + "','YYYY/MM/DD') ";
            }
            if (mo.endtime != null)
            {
                sql += "and CREATETIME<=to_date('" + mo.endtime + "','YYYY/MM/DD') ";
            }

            return sql;

        }
        public Check_Model CHECK_GetModelFromDataReader(OracleDataReader dr)
        {
        ...
        public bool GetCheckArea(int hl, string areano, string houseno, string warehouseno, ref List<CheckArea_Model> list)
        {
            ...
            DataTable dt = OracleDBHelper.ExecuteDataSet(System.Data.CommandType.Text, sql).Tables[0];
        ...
        public bool SaveCheck(Check_Model model, List<CheckArea_Model> list, ref string ErrMsg)
        {
            try
            {
                ...
                string sqlhead = "insert into T_CHECK(ID,CHECKNO,CHECKTYPE,CHECKDESC,CHECKSTATUS,REMARKS,ISDEL,CREATER,CREATETIME) VALUES" +
                    "(" + id + ",'" + model.CHECKNO + "','" + model.CHECKTYPE + "','" + model.CHECKDESC + "','" + model.CHECKSTATUS + "','" + model.REMARKS + "',1,'" + model.CREATER + "',SYSDATE)";
                sqls.Add(sqlhead);
                string sql = "";
                ...
                int i = OracleDBHelper.ExecuteNonQueryList(sqls);
                if (i == -2)
                    return false;
                else
                    return true;
        ...
        public bool SaveCheck2(Check_Model model, List<T_StockInfoEX> list, ref string ErrMsg)
        {
            try
            {
                ...
                List<string> sqls = new List<string>();
                string sqlhead = "insert into T_CHECK(ID,CHECKNO,CHECKTYPE,CHECKDESC,CHECKSTATUS,REMARKS,ISDEL,CREATER,CREATETIME) VALUES" +
                    "(" + id + ",'" + checkno + "','" + "明盘" + "','" + model.CHECKDESC + "','" + model.CHECKSTATUS + "','" + model.REMARKS + "',1,'" + model.CREATER + "',SYSDATE)";
                sqls.Add(sqlhead);
                string sql = "";
                //合并重复项
                list = list
                ...
                foreach (var item in list)
                {
                    sql = "insert into T_checkrefstock (id,voucherno,MaterialNo,WAREHOUSENO,HouseNo,AreaNo,qty,STRONGHOLDCODE,BATCHNO,sqty) " +
                        "values(seq_checkrefstock_id.nextval,'" + checkno + "','" + item.MaterialNo + "','" + item.WarehouseNo + "','" + item.HouseNo + "','" + item.AreaNo + "'," + item.Qty + ",'" + item.StrongHoldCode + "','" + item.BatchNo + "',0)";
                    sqls.Add(sql);
                }
                int i = OracleDBHelper.ExecuteNonQueryList(sqls);
                if (i == -2)
        ...
        public bool ReCheck(string checkno, string peo, ref string ErrMsg)
        {
            try
            {
                ...
                DataTable dt = OracleDBHelper.ExecuteDataSet(CommandType.Text, sql).Tables[0];
                ...
                using (var dr = OracleDBHelper.ExecuteReader(sql))
                {
                    while (dr.Read())
                    {
        ...
        public bool GetCheckRefInfo(CheckRef_Model model, ref DividPage dividpage, ref List<CheckRef_Model> lsttask, ref string strErrMsg)
        {
            ...
            try
            {
                using (OracleDataReader dr = Common_DB.QueryByDividPage(ref dividpage, "V_CHECKREF", CHECKREF_GetFilterSql(model), " * ", "order by CHECKID"))
                {
                    while (dr.Read())
                    {
        ...
        public CheckRef_Model CHECKREF_GetModelFromDataReader(OracleDataReader dr)
        {
        ...
        public bool DelCloCheck(string checkno, int type, ref string ErrMsg, string username = "")
        {
            try
            {
                ...
                if (type == 0)
                {
                    ...
                    int cn = OracleDBHelper.ExecuteScalar(CommandType.Text, sql).ToInt32();
                    if (cn != 0)
                    {
                    ...
                    foreach (var item in listshow)
                    {
                        ...
                        if (item.remark == "赢")
                        {
                            ...
                            if (String.IsNullOrEmpty(item.SSERIALNO))
                            {
                                ...
                                int o = Convert.ToInt32(OracleDBHelper.ExecuteScalar(CommandType.Text, sql));
                                if (o > 0)
                                {
                                ...
                                else
                                {
                                    ...
                                    sql = "insert into T_STOCK (ID,BARCODE,SERIALNO,MATERIALNO,MATERIALDESC,MATERIALNOID,WAREHOUSEID," +
                                       ...
                                       ",to_date('" + item.PRODUCTDATE.ToString("yyyy-MM-dd") + "','YYYY/MM/DD'),'" + item.SUPPRDBATCH + "',to_date('" + item.SUPPRDDATE.ToString("yyyy-MM-dd") + "','YYYY/MM/DD'),2,2,'"+ item.ean + "')";
                                    sqls.Add(sql);
                                    sql = InTrans(username, sql, item, 101, item.SERIALNO);
                                    sqls.Add(sql);
                                }
                ...
                //执行
                int i = OracleDBHelper.ExecuteNonQueryList(sqls, ref ErrMsg);
                if (i == -2)
                    return false;
                else
                    return true;
        ...
        public bool DelCloCheck_Ms(string checkno, int type, ref string ErrMsg, string username = "")
        {
            try
            {
                ...
                if (type == 2)
                {
                    ...
                    "select NVL(a.sserialno, a.serialno) as ss, NVL(a.yqty, 0) as yqty, NVL(a.kqty, 0) as kqty, NVL(a.smaterialno, a.materialno) as materialno from( " +
                    ...
                    "b.STRONGHOLDCODE as SSTRONGHOLDCODE, b.AREANO as SAREANO, b.MATERIALNO as SMATERIALNO, b.QTY as SQTY, b.MATERIALDESC as SMATERIALDESC, b.SERIALNO as SSERIALNO," +
                    "case when nvl(a.QTY, 0) < nvl(b.QTY, 0) then '亏' when nvl(a.QTY, 0) > nvl(b.QTY, 0)  then '赢' else '平' end as remark,case when nvl(a.QTY,0)> nvl(b.QTY, 0) then to_char(nvl(a.QTY,0)-nvl(b.QTY, 0)) else '' end as YQTY,case when nvl(a.QTY,0)< nvl(b.QTY, 0) then to_char(nvl(b.QTY,0)-nvl(a.QTY, 0)) else '' end as KQTY " +
                    ...
                    DataTable dtAll = OracleDBHelper.ExecuteDataSet(System.Data.CommandType.Text, sqlall).Tables[0];
        ...
        {
            sql = "insert into T_TASKTRANS (VOUCHERTYPE,ID,MATERIALNO,MATERIALDESC,CREATETIME,UNIT,CREATER,MATERIALNOID,STRONGHOLDNAME,COMPANYCODE" +
                ...
                " select " + vouchertype + ",seq_tasktrans_id.nextval,f.materialno,f.materialdesc,sysdate,f.unit,'" + username + "',f.materialnoid,f.strongholdname,10, " +
        ...
        public CheckAnalyze GetCheckStock_GetModelFromDataReader(OracleDataReader dr)
        {
            CheckAnalyze TMM = new CheckAnalyze();
            TMM.CHECKNO = (dr["CHECKNO"] ?? "").ToString();
            TMM.AREANO = (dr["AREANO"] ?? "").ToString();
        ...
        //    OracleDBHelper.ExecuteNonQuery(CommandType.Text, sql);
        ...
        public string GetMinBarocde(string barcode, string checkno)
        {
            ...
            if (BarCodeType == "1" || (BarCodeType == "3" && !SerialNo.Contains('P')))
            {
                ...
                int c = Convert.ToInt32(OracleDBHelper.ExecuteScalar(CommandType.Text, sql));
                if (c > 0)
                {
                    BaseMessage_Model<List<CheckDet_Model>> bm = new BaseMessage_Model<List<CheckDet_Model>>();
                    bm.HeaderStatus = "E";
                ...
                sqls.Add(sql);
                OracleDBHelper.ExecuteNonQueryList(sqls);
        ...
        public string InsertCheckDetail(string json)
        {
            try
            {
                ...
                foreach (Barcode_Model model in list)
                {
                    ...
                    {
                        //修改数据
                        sql = "update T_CHECKDETAILS set CREATER = '" + model.Creater + "', AREAID = " + model.AREAID + ",qty = " + model.Qty + ",INSERTTIME = SYSDATE,status = " + model.STATUS + " where CHECKNO = '" + model.CHECKNO + "' and SERIALNO = '" + model.SerialNo + "' ";
                        sqls.Add(sql);
                    }
                    else
                    {
                        sql = "insert into T_CHECKDETAILS(CREATER,CHECKNO,AREAID,MATERIALID,SERIALNO,QTY,INSERTTIME,status) values('" + model.Creater + "','" + model.CHECKNO + "'," + model.AREAID + "," + model.MaterialNoID + ",'" + model.SerialNo + "'," + model.Qty + ",SYSDATE," + model.STATUS + ")";
                        sqls.Add(sql);
                    }
                }
                int val = OracleDBHelper.ExecuteNonQueryList(sqls);
                if (val == -2)
                {
                    BaseMessage_Model<string> bm = new BaseMessage_Model<string>();
                    bm.HeaderStatus = "E";
        ...
        public string GetScanInfo(string barcode)
        {
            ...
            if (BarCodeType == "2" || (BarCodeType == "3" && SerialNo.Contains('P')))
            {
                ...
                //-----------------------原本托盘点-------------------
                //sql = "select serialno from T_PALLETDETAIL where pallettype = 1 and palletno = '" + SerialNo + "'";
                //sql = "select o.*,s.qty as sqty,s.MaterialNoID as sMaterialNoID,s.StrongHoldCode  as SStrongHoldCode,s.StrongHoldName as SStrongHoldName  from t_outbarcode o left join t_stock s on o.serialno = s.serialno  where o.SERIALNO in (" + sql + ")";
                //using (var dr = OracleDBHelper.ExecuteReader(sql))
                //{
                //    while (dr.Read())
                //    {
        ...
        private static Barcode_Model MakeQInfo(OracleDataReader dr)
        {
        ...
        private static Barcode_Model MakeQInfo2(OracleDataReader dr)
        {
            Barcode_Model bm = new Barcode_Model();
            bm.SerialNo = dr["SerialNo"].ToDBString();
            bm.BarCode = dr["BarCode"].ToDBString();
        ...
        private static Barcode_Model MakeQInfo3(OracleDataReader dr)
        {
            Barcode_Model bm = new Barcode_Model();
        ...
        private static Barcode_Model MakeQInfo4(OracleDataReader dr)
        {
            Barcode_Model bm = new Barcode_Model();
            //bm.areano = dr["areano"].ToDBString();
            bm.SerialNo = dr["SerialNo"].ToDBString();
        ...
        public string TakeAID(string json)
        {
            ...
            using (var dr = OracleDBHelper.ExecuteReader(sql))
            {
                ...
                else
                {
                    ...
                    object o = OracleDBHelper.ExecuteScalar(CommandType.Text, sql);
        ...
        public string SaveInfo(string json, string man)
        {
            try
            {
                ...
                man = OracleDBHelper.ExecuteScalar(CommandType.Text, sqlname).ToDBString();
                List<string> sqls = new List<string>();
                string sql = "";
                List<Barcode_Model> list = Check_Func.DeserializeJsonToList<Barcode_Model>(json);
                ...
                if (bar.AllIn == "1")
                {
                    ...
                    using (var dr = OracleDBHelper.ExecuteReader(CommandType.Text, sql))
                    {
                        if (dr.Read())
                        {
                    ...
                    sql = "insert into T_TASKTRANS (VOUCHERTYPE,ID,MATERIALNO,MATERIALDESC,CREATETIME,UNIT,CREATER,MATERIALNOID,STRONGHOLDNAME,COMPANYCODE" +
                           " ,STRONGHOLDCODE,BATCHNO,PRODUCTDATE,SUPPRDBATCH,SUPPRDDATE,SERIALNO" +
                            " ,TOWAREHOUSEID ,TOHOUSEID ,TOAREAID " +
                          "  , BARCODE ,TASKTYPE ,QTY ,EDATE ,FROMWAREHOUSEID , FROMHOUSEID ,FROMAREAID,status)" +
                            "select 100,seq_tasktrans_id.nextval,f.materialno,f.materialdesc,sysdate,f.unit,'" + man + "',f.materialnoid,f.strongholdname,10, " +
                          "  f.strongholdcode,f.batchno,f.productdate,f.supprdbatch,f.supprddate,f.serialno" +
                           " ,'','','',f.barcode,2,f.qty,f.edate,f.warehouseid,f.houseid,f.areaid,f.status from t_stock f where f.barcode = '" + bar.BarCode + "'";
                    sqls.Add(sql);
                    //修改库存表
                    sql = "update t_stock set edate =to_date('" + bar.EDate.ToString("yyyy/MM/dd") + "','yyyy/mm/dd') ,materialnoid=" + mid + ",qty = " + bar.Qty + ",BatchNo ='" + bar.BatchNo + "',StrongHoldCode ='" + bar.StrongHoldCode + "',StrongHoldName ='" + bar.StrongHoldName + "',areaid = " + areaid + ",houseid = " + houseid + ",warehouseid = " + warehouseid + ",status=" + bar.STATUS + " where serialno = '" + bar.SerialNo + "'";
                    sqls.Add(sql);
                    sql = "insert into T_TASKTRANS (VOUCHERTYPE,ID,MATERIALNO,MATERIALDESC,CREATETIME,UNIT,CREATER,MATERIALNOID,STRONGHOLDNAME,COMPANYCODE" +
                        " ,STRONGHOLDCODE,BATCHNO,PRODUCTDATE,SUPPRDBATCH,SUPPRDDATE,SERIALNO" +
                         " ,TOWAREHOUSEID ,TOHOUSEID ,TOAREAID " +
                       "  , BARCODE ,TASKTYPE ,QTY ,EDATE ,FROMWAREHOUSEID , FROMHOUSEID ,FROMAREAID,status)" +
                         "select 100,seq_tasktrans_id.nextval,f.materialno,f.materialdesc,sysdate,f.unit,'" + man + "',(select id from t_material t where t.materialno = '" + bar.MaterialNo + "' and t.strongholdcode='" + bar.StrongHoldCode + "'),'" + bar.StrongHoldName + "',10, " +
                       " '" + bar.StrongHoldCode + "','" + bar.BatchNo + "',f.productdate,f.supprdbatch,f.supprddate,f.serialno" +
                        " ," + warehouseid + "," + houseid + "," + areaid + ",f.barcode,1," + bar.Qty + ",f.edate,'','','',f.status from t_stock f where f.barcode = '" + bar.BarCode + "'";
                    sqls.Add(sql);
                    sql = "update t_outbarcode set materialnoid=(select id from t_material t where t.materialno = '" + bar.MaterialNo + "' and t.strongholdcode='" + bar.StrongHoldCode + "'),edate =to_date('" + bar.EDate.ToString("yyyy/MM/dd") + "','yyyy/mm/dd'),qty = " + bar.Qty + ",BatchNo ='" + bar.BatchNo + "',StrongHoldCode ='" + bar.StrongHoldCode + "',StrongHoldName ='" + bar.StrongHoldName + "' where serialno = '" + bar.SerialNo + "'";
                    sqls.Add(sql);
                    //插入流水
                ...
                else if (bar.AllIn == "0")
                {
                    ...
                    sql = "insert into T_STOCK (ID,BARCODE,SERIALNO,MATERIALNO,MATERIALDESC,MATERIALNOID,WAREHOUSEID," +
                                          ...
                                          "," + item.Qty + "," + bar.STATUS + ",1,'" + man + "',SYSDATE,'" + item.BatchNo + "','" + item.Unit + "','" + item.PalletNo + "','" + item.StrongHoldCode + "','" + item.StrongHoldName + "'" +
                                          ",'10',to_date('" + item.EDate.ToString("yyyy-MM-dd") + "','YYYY/MM/DD'),'" + item.SupCode + "','" + item.SupName + "'" +
                                          ",to_date('" + item.ProductDate.ToString("yyyy-MM-dd") + "','YYYY/MM/DD'),'" + item.SupPrdBatch + "',to_date('" + item.SupPrdDate.ToString("yyyy-MM-dd") + "','YYYY/MM/DD'),2,2)";
                    sqls.Add(sql);
                    //插入流水
                    sql = "insert into T_TASKTRANS (VOUCHERTYPE,ID,MATERIALNO,MATERIALDESC,CREATETIME,UNIT,CREATER,MATERIALNOID,STRONGHOLDNAME,COMPANYCODE" +
                         ...
                         "  , BARCODE ,TASKTYPE ,QTY ,EDATE ,FROMWAREHOUSEID ,FROMHOUSEID ,FROMAREAID,status)" +
                           "select 100,seq_tasktrans_id.nextval,f.materialno,f.materialdesc,sysdate,f.unit,'" + man + "',f.materialnoid,f.strongholdname,10, " +
                         "  f.strongholdcode,f.batchno,f.productdate,f.supprdbatch,f.supprddate,f.serialno" +
                          " ,f.warehouseid,f.houseid,f.areaid,f.barcode,1,f.qty,f.edate,'','','',f.status from t_stock f where f.barcode = '" + bar.BarCode + "'";
                    sqls.Add(sql);
                ...
                else
                {
                    ...
                    if (!string.IsNullOrEmpty(bar.areano))
                    {
                        ...
                        sqls.Add(sql);
                        //插入放颖库存备份表
                        sql = "INSERT INTO t_Stocktrans (Id, Barcode, Serialno, Materialno, Materialdesc, Warehouseid, Houseid, Areaid, Qty,  Status, Isdel, Creater, Createtime, " +
                            ...
                            "A.Supprddate,A.Isquality FROM T_STOCK A WHERE A.barcode = '" + bar.BarCode + "'";
                        sqls.Add(sql);
                ...
                string errMsg = "";
                OracleDBHelper.ExecuteNonQueryList(sqls, ref errMsg);
                ...
                if (!string.IsNullOrEmpty(palletno))
                {
                    ...
                    int i = OracleDBHelper.ExecuteScalar(CommandType.Text, sql).ToInt32();
                    if (i == 0)
                    {
                        //没有托盘子信息，删除托盘头
