    ...
    public class OutStockCreate_SplitFclRule:OutStockCreate_SplitBaseRule<T_OutStockCreateInfo, T_StockInfo>
    {
        public override void GetOutStockCreateSplitList(ref List<T_OutStockCreateInfo> modelList, List<T_StockInfo> stockList)
        {
            ...
            foreach (var item in modelList)
            {
                ...
                //物料整箱区库存
                materialFclStock = stockList.FindAll(t => t.MaterialNoID == item.MaterialNoID && t.HouseProp == 1 && t.StrongHoldCode == item.StrongHoldCode && t.WarehouseNo == item.FromErpWareHouse);
                

                //整箱区库存为零或者物料包装量为零或者库存零拣区数量< 订单零拣数量，直接从整箱区拣货
                //if (materialFclStock == null || materialFclStock.Count==0 || item.PackQty == 0 )
                //{
                //    //物料整箱区库存为零，默认分配到整箱区
