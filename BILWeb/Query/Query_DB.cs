...
namespace BILWeb.Query
{
    ...
    public class Query_DB
    {
        ...
        public bool GetStockCombineInfo(T_StockInfoEX taskmo, ref DividPage dividpage, ref List<T_StockInfoEX> lsttask, ref string strErrMsg)
        {
            ...
            try
            {
                using (OracleDataReader dr = Common_DB.QueryByDividPage(ref dividpage, getsqlinStockCombine(taskmo), StockC_GetFilterSql(taskmo), " * ", "Order by ID Desc"))
                {
                    while (dr.Read())
                    {
        ...
        public T_StockInfoEX StockC_GetModelFromDataReader(OracleDataReader dr)
        {
            T_StockInfoEX TMM = new T_StockInfoEX();
            TMM.XH = ++xh + "";
            ...
            TMM.WarehouseNo = (dr["WarehouseNo"] ?? "").ToString();
            TMM.HouseNo = (dr["HouseNo"] ?? "").ToString();
            TMM.AreaNo = (dr["AreaNo"] ?? "").ToString();
        ...
        public bool GetTaskTransInfo(TaskTrans_Model taskmo, ref DividPage dividpage, ref List<TaskTrans_Model> lsttask, ref string strErrMsg)
        {
            ...
            try
            {
                using (OracleDataReader dr = Common_DB.QueryByDividPage(ref dividpage, "V_TaskTran", TASKTRANS_GetFilterSql(taskmo), " * ", "Order by ID Desc"))
                {
                    while (dr.Read())
                    {
        ...
        public TaskTrans_Model TASKTRANS_GetModelFromDataReader(OracleDataReader dr, List<VoucherType> lstvou)
        {
            TaskTrans_Model TMM = new TaskTrans_Model();
            TMM.Edate = dr["edate"].ToDateTime();
            TMM.XH = ++xh + "";
        ...
        public bool GetAllQitao(qitao_Model model, ref DividPage dividpage, ref List<qitao_Model> lsttask, ref string strErrMsg)
        {
            ...
            try
            {
                ...
                DataTable dt = OracleDBHelper.ExecuteDataSet(CommandType.Text, strb.ToString()).Tables[0];
        ...
        public bool GetHistory(Linehistory model, ref DividPage dividpage, ref List<Linehistory> lsttask, ref string strErrMsg)
        {
            ...
            try
            {
                ...
                strb.Append(" join t_linemanagemodel d  on a.linemanageno = d.id) dd  group by erpvoucherno, productteamno) ab left join(select erpvoucherno, productteamno, nvl(sum(postnum), 0) postnum from(select erpvoucherno, productteamno, postnum from(select a.*, d.productteamno from t_pro_linemanagehistory a left ");
        ...
        public bool GetHistoryForLook(ref List<Linehistory> lsttask, ref string strErrMsg)
        {
            ...
            try
            {
                ...
                DataTable dt = OracleDBHelper.ExecuteDataSet(CommandType.Text, aaa).Tables[0];
