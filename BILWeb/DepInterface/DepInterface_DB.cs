...
{
    public partial class T_DepInterface_DB : BILBasic.Basing.Base_DB<T_DepInterfaceInfo>
    {

        /// <summary>
        /// 添加t_depinterface
        /// </summary>
        protected override OracleParameter[] GetSaveModelOracleParameter(T_DepInterfaceInfo t_depinterface)
        {
        ...
        /// 将获取的单条数据转封装成对象返回
        /// </summary>
        protected override T_DepInterfaceInfo ToModel(OracleDataReader reader)
        {
            T_DepInterfaceInfo t_depinterface = new T_DepInterfaceInfo();

            t_depinterface.ID = OracleDBHelper.ToModelValue(reader, "ID").ToInt32();
            t_depinterface.VoucherName = OracleDBHelper.ToModelValue(reader, "VOUCHERNAME").ToInt32();
            t_depinterface.VoucherType = OracleDBHelper.ToModelValue(reader, "VOUCHERTYPE").ToInt32();
            t_depinterface.Function = OracleDBHelper.ToModelValue(reader, "FUNCTION").ToInt32();
            t_depinterface.DLLName = (string)OracleDBHelper.ToModelValue(reader, "DLLNAME");
            t_depinterface.ClassName = (string)OracleDBHelper.ToModelValue(reader, "ClassName");
            t_depinterface.Route = (string)OracleDBHelper.ToModelValue(reader, "ROUTE");
            t_depinterface.FunctionName = (string)OracleDBHelper.ToModelValue(reader, "FUNCTIONNAME");
            t_depinterface.FunctionNote = (string)OracleDBHelper.ToModelValue(reader, "FUNCTIONNOTE");
            t_depinterface.Creater = (string)OracleDBHelper.ToModelValue(reader, "CREATER");
            t_depinterface.CreateTime = (DateTime?)OracleDBHelper.ToModelValue(reader, "CREATETIME");
            t_depinterface.Modifyer = (string)OracleDBHelper.ToModelValue(reader, "MODIFYER");
            t_depinterface.ModifyTime = (DateTime?)OracleDBHelper.ToModelValue(reader, "MODIFYTIME");
            t_depinterface.StrVoucherName = OracleDBHelper.ToModelValue(reader, "STRVOUCHERNAME").ToDBString();
            t_depinterface.StrFunction = OracleDBHelper.ToModelValue(reader, "strFunction").ToDBString();
            t_depinterface.StrVoucherType = OracleDBHelper.ToModelValue(reader, "StrVoucherType").ToDBString();
