...
{
    public partial class T_PalletDetail_DB : BILBasic.Basing.Base_DB<T_PalletDetailInfo>
    {

        /// <summary>
        /// 添加t_palletdetail
        /// </summary>
        protected override OracleParameter[] GetSaveModelOracleParameter(T_PalletDetailInfo t_palletdetail)
        {
            ...
            else 
            {
                ...
                foreach (var item in modelList)
                {
                    ...
                    foreach (var itemSerial in item.lstBarCode)
                    {
                        ...
                        "'{16}','{17}',to_date('{18}','yyyy-mm-dd hh24:mi:ss'),'{19}',to_date('{20}','yyyy-mm-dd hh24:mi:ss'),'{21}',to_date('{22}','yyyy-mm-dd hh24:mi:ss'),'{23}','{24}','{25}')", detailID, ID, VoucherNo, itemSerial.MaterialNo, itemSerial.MaterialDesc, itemSerial.SerialNo, user.UserNo,
        ...
        private List<string> GetInsertPalletNewSerialNoSql(UserModel user,List<T_PalletDetailInfo> modelList)
        {
            ...
            foreach (var item in modelList)
            {
                foreach (var itemSerial in item.lstBarCode)
                {
                    strSql = "insert into t_Palletdetail (Id, Headerid, Palletno, Materialno, Materialdesc, Serialno, Creater, Createtime, Unit," +
                            ...
                            "sysdate,'" + itemSerial.Unit + "','" + itemSerial.RowNo + "','" + itemSerial.VoucherNo + "',  '" + itemSerial.ErpVoucherNo + "','" + itemSerial.MaterialNoID + "','" + itemSerial.Qty + "'," +
        ...
        /// 将获取的单条数据转封装成对象返回
        /// </summary>
        protected override T_PalletDetailInfo ToModel(OracleDataReader reader)
        {
            T_PalletDetailInfo t_palletdetail = new T_PalletDetailInfo();

            t_palletdetail.ID = OracleDBHelper.ToModelValue(reader, "ID").ToInt32();
            t_palletdetail.HeaderID = OracleDBHelper.ToModelValue(reader, "HEADERID").ToInt32();
            t_palletdetail.PalletNo = OracleDBHelper.ToModelValue(reader, "PALLETNO").ToDBString();
            t_palletdetail.MaterialNo = OracleDBHelper.ToModelValue(reader, "MATERIALNO").ToDBString();
            t_palletdetail.MaterialDesc = OracleDBHelper.ToModelValue(reader, "MATERIALDESC").ToDBString();
            t_palletdetail.SerialNo = OracleDBHelper.ToModelValue(reader, "SERIALNO").ToDBString();
            t_palletdetail.MaterialNoID = OracleDBHelper.ToModelValue(reader, "MATERIALNOID").ToInt32();
            t_palletdetail.ErpVoucherNo = OracleDBHelper.ToModelValue(reader, "ERPVoucherNo").ToDBString();
            t_palletdetail.BarCode = OracleDBHelper.ToModelValue(reader, "BarCode").ToDBString();
            t_palletdetail.StrongHoldCode = OracleDBHelper.ToModelValue(reader, "StrongHoldCode").ToDBString();
            t_palletdetail.StrongHoldName = OracleDBHelper.ToModelValue(reader, "StrongHoldName").ToDBString();
            t_palletdetail.CompanyCode = OracleDBHelper.ToModelValue(reader, "CompanyCode").ToDBString();
            t_palletdetail.PalletType = OracleDBHelper.ToModelValue(reader, "pallettype").ToInt32();
            t_palletdetail.Qty = (decimal?)OracleDBHelper.ToModelValue(reader, "Qty");
            t_palletdetail.BatchNo = OracleDBHelper.ToModelValue(reader, "BatchNo").ToDBString();
            t_palletdetail.RowNo = OracleDBHelper.ToModelValue(reader, "RowNo").ToDBString();
            t_palletdetail.RowNoDel = OracleDBHelper.ToModelValue(reader, "RowNoDel").ToDBString();
            t_palletdetail.ProductBatch = OracleDBHelper.ToModelValue(reader, "ProductBatch").ToDBString();
            t_palletdetail.ProductDate = OracleDBHelper.ToModelValue(reader, "ProductDate").ToDateTime();
            t_palletdetail.SupPrdBatch = OracleDBHelper.ToModelValue(reader, "SupPrdBatch").ToDBString();
            t_palletdetail.SupPrdDate = OracleDBHelper.ToModelValue(reader, "SupPrdDate").ToDateTime();

            t_palletdetail.SuppliernNo = OracleDBHelper.ToModelValue(reader, "SupplierNo").ToDBString();
            t_palletdetail.SupplierName = OracleDBHelper.ToModelValue(reader, "SupplierName").ToDBString();
            t_palletdetail.Unit = OracleDBHelper.ToModelValue(reader, "Unit").ToDBString();

            //add by cym 2019-3-26 完工入库的时候需要传效期日！！！
            t_palletdetail.EDate = OracleDBHelper.ToModelValue(reader, "EDate").ToDateTime();
            t_palletdetail.EAN = OracleDBHelper.ToModelValue(reader, "EAN").ToDBString();
        ...
        public bool CheckDeletePalletDetailBefore(List<T_PalletDetailInfo> modelList, ref string strError) 
        {
                ...
                OracleParameter[] cmdParms = new OracleParameter[] 
            {
                ...
                OracleDBHelper.ExecuteNonQuery2(OracleDBHelper.ConnectionStringLocalTransaction, CommandType.StoredProcedure, "p_checkpalletisdelete", cmdParms);
        ...
        private List<string> GetUnBoxAmountSql(T_StockInfo model, UserModel userModel,ref string NewSerialNo) 
        {
            ...
            strSql = "update t_stock a set a.Qty = a.Qty - '" + model.AmountQty + "'  where serialno = '" + model.SerialNo + "'";
            lstSql.Add(strSql);
            ...
            //    strSql = "update t_Palletdetail a set a.Qty = a.qty - '" + model.AmountQty + "' where serialno = '" + model.SerialNo + "'";
            //    lstSql.Add(strSql);

            //    strSql = "delete t_Palletdetail a where a.Serialno = '" + model.SerialNo + "' and a.Qty = 0";
            //    lstSql.Add(strSql);
            //}
        ...
        private List<string> GetBoxAmountSql(UserModel userModel,T_StockInfo OldModel,T_StockInfo NewModel )
        {
            ...
            if (!string.IsNullOrEmpty(OldModel.PalletNo))
            {
                ...
                strSql = "update t_Palletdetail a set a.Qty = a.qty + '" + NewModel.Qty + "' where serialno = '" + OldModel.SerialNo + "'";
                lstSql.Add(strSql);

                strSql = "delete t_Palletdetail a where a.Serialno = '" + NewModel.SerialNo + "' and a.Qty = 0";
                lstSql.Add(strSql);
            }
        ...
        public bool GetBarCodeByPalletDetail(List<T_PalletDetailInfo> modelList,ref List<T_OutBarCodeInfo> lstBarCode, ref string strError) 
        {
                ...
                OracleParameter[] cmdParms = new OracleParameter[] 
            {
                ...
                ds = OracleDBHelper.ExecuteDataSetForCursor(ref iResult, ref strError, OracleDBHelper.ConnectionStringLocalTransaction, CommandType.StoredProcedure, "p_getbarcodebypalletdetail", cmdParms);
        ...
        public bool GetBarCodeByPalletDetailnew(List<T_PalletDetailInfo> modelList, ref List<T_OutBarCodeInfo> lstBarCode, ref string strError)
        {
                ...
                OracleParameter[] cmdParms = new OracleParameter[]
            {
                ...
                ds = OracleDBHelper.ExecuteDataSetForCursor(ref iResult, ref strError, OracleDBHelper.ConnectionStringLocalTransaction, CommandType.StoredProcedure, "p_getbarcodedetails", cmdParms);
        ...
        public bool CheckSerialNoIsMore(string json)
        {
            try
            {
                if (json.Substring(0, 2) == "2@"){
                    ...
                    using (OracleDataReader dr = OracleDBHelper.ExecuteReader(System.Data.CommandType.Text, strSql1))
                    {
                        while (dr.Read())
                        {
                    ...
                    using (OracleDataReader dr = OracleDBHelper.ExecuteReader(System.Data.CommandType.Text, strSql2))
                    {
                        while (dr.Read())
                        {
                            if (dr["aa"].ToInt32() > 1)
        ...
        public bool CreatePalletByEmsLabel(List<T_OutStockDetailInfo> modelList,int PrintBoxQty ,UserModel user, ref string strPalletNo, ref string strError)
        {
            try
            {
                ...
                strSql = string.Format("insert into t_Pallet(Id, Palletno, Creater, Createtime,Strongholdcode,Strongholdname,Companycode,Pallettype,Supplierno,Suppliername,ERPVOUCHERNO,BOXCOUNT)" +
                                " values ('{0}','{1}','{2}',Sysdate,'{3}','{4}','{5}','{6}','{7}','{8}','{9}','{10}')", ID, VoucherNo, user.UserNo, modelList[0].StrongHoldCode, modelList[0].StrongHoldName, modelList[0].CompanyCode, '4', modelList[0].CustomerCode, modelList[0].CustomerName, modelList[0].ErpVoucherNo, PrintBoxQty);
